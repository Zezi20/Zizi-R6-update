import HighQualityFeature from "../components/HighQualityFeature"; // Correct import path
import React, { Suspense } from "react";

// Data array for High Quality features
const data = [
  {
  ...
  {
    trophyImage: "images/img_shipping.svg",
...
export default function ContactRowOne() {
  return (
    <div>
      <div className="bg-lime-50 px-[52px] py-[100px] md:p-5">
        <div className="flex gap-[54px] md:flex-col">
          <Suspense fallback={<div>Loading feed...</div>}>
            {data.map((d, index) => (
              <HighQualityFeature {...d} key={"listhigh" + index} className="w-[28%]" />
            ))}
          </Suspense>
        </div>
      </div>
